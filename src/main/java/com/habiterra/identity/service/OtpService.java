...
public class OtpService {
    ...
    public RequestOtpResponse request(String input) {
        ...
        var previous =
                otps.findFirstByIdentifierAndIdentifierTypeOrderByIdDesc(
                        id.value(),
                        id.type()
                );
        ...
        /*
         * MODE DEV :
         * on n'appelle pas le fournisseur SMS.
         * L'OTP est affiché dans les logs.
         */
        if (devMode) {

            log.warn(
                    "[OTP DEV] Identifier: {} | Code: {}",
                    id.value(),
                    code
            );
    ...
    public VerifyOtpResponse verify(
        ...
        if (
            ...
            throw new AuthException(
                    otp.getAttempts() >= otp.getMaxAttempts()
                            ? 429
                            : 400,
                    otp.getAttempts() >= otp.getMaxAttempts()
                            ? "OTP_MAX_ATTEMPTS"
